defmodule ReportRepair do
  def fix_report(file_path) do
    ...
    possible_trios = cartesian_product(expenses, expenses, expenses)
    result = check_sum(possible_trios)

    case result do
      {:ok , {x, y, z}} -> {{x, y, z}, x * y * z}
  ...
  defp cartesian_product(a, b, c) do
    for x <- a, y <- b, z <- c, do: {x, y, z}
  end

  defp check_sum([head | tail]) do
    case head do
      {x, y, z} when x + y + z == 2020 ->
        {:ok, head}
      _ ->
        check_sum(tail)
    end
