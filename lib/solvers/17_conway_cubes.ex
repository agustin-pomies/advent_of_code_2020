defmodule ConwayCubes do
  ...
  def execute_cycle_3d(board) do
    active_neighbours =
      Enum.reduce(board, %{}, fn {coords, state}, acc ->
        if state == :active, do: update_neighbours(acc, neighbours_3d(coords)), else: acc
      end)

    for {coords, neighbours} <- active_neighbours,
        into: %{},
        do: {coords, new_state(board, coords, neighbours)}
  end
  ...
  def update_neighbours(neighbours_register, [coord | coords]) do
    case Map.fetch(neighbours_register, coord) do
      {:ok, neighbours} ->
        update_neighbours(Map.put(neighbours_register, coord, neighbours + 1), coords)

      :error ->
        update_neighbours(Map.put(neighbours_register, coord, 1), coords)
    end
  end
  ...
  def execute_cycle_4d(board) do
    active_neighbours =
      Enum.reduce(board, %{}, fn {coords, state}, acc ->
        if state == :active, do: update_neighbours(acc, neighbours_4d(coords)), else: acc
      end)
