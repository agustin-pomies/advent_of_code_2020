defmodule AdapterArray do
  ...
  defmemo auxiliar([adapter | tail]) do
    ...
    |> Enum.map(fn previous_adapter ->
      {previous_adapter, Enum.drop_while(tail, fn elem -> elem != previous_adapter end)}
    end)
    |> Enum.reduce(0, fn {_previous_adapter, subsubcollection}, acc ->
      acc + count_path(subsubcollection)
    end)
  end
end
