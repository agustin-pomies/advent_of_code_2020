defmodule TobogganTrajectory do
  ...
  def part_one do
    get_data() |> tree_encounters({3, 1})
  end

  def part_two do
    board = get_data()
    tree_encounters = Enum.map(slopes(), &tree_encounters(board, &1))
    Enum.reduce(tree_encounters, 1, fn x, acc -> x * acc end)
  end
  ...
  def parse_board(rows) do
    ...
    |> Enum.map(fn row ->
      Enum.map(row, fn symbol -> elem(Map.fetch(tile_type(), symbol), 1) end)
    end)
  end
  ...
  def begin_trajectory(board, {x, y}, movement, tree_encounters \\ 0) do
    ...
    case result do
      {:halt, result_tree_encounters} ->
        result_tree_encounters

      {:repeat_board, {new_x, new_y}, result_tree_encounters} ->
        begin_trajectory(board, {new_x, length(board) - new_y}, movement, result_tree_encounters)
    end
  end
  ...
  def travel_through_board(movement, [_ | [_ | _]] = board, tree_encounters) do
    case apply_movement(board, movement) do
      {:travel, new_board} ->
        travel_through_board(movement, new_board, new_tree_encounters(new_board, tree_encounters))

      {:change_board, initial_square} ->
        {:repeat_board, initial_square, tree_encounters}

      {:halt, final_board} ->
        {:halt, new_tree_encounters(final_board, tree_encounters)}
    end
  end
