defmodule ComboBreaker do
  ...
  def part_one do
    ...
    secret_loops = Enum.map(public_keys, fn public_key -> loop({1, public_key}, @initial_subject_number, 0) end)
  ...
  def loop({current_number, objective}, _, loop_size) when current_number == objective, do: loop_size
