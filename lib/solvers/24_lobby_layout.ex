defmodule LobbyLayout do
  ...
  def neighbors({a, b}) do
    for x <- [a-1.0, a-0.5, a+0.5, a+1.0], y <- [b-1.0, b, b+1.0], (abs(y - b) == 0.0 || abs(x - a) == 0.5) && (abs(y - b) != 0.0 || abs(x - a) == 1.0), do: {x, y}
  end

  def tiles_to_flip(floor, tiles_to_examine) do
    tiles_to_examine
    |> Enum.filter(fn {coordinates, number_of_neighbours} -> need_state_change?(Map.get(floor, coordinates, :white), number_of_neighbours) end)
    |> Enum.map(&(elem(&1, 0)))
  end
  ...
  def floor_after_flip(tiles_to_flip, current_floor) do
    Enum.reduce(tiles_to_flip, current_floor, fn elem, acc -> Map.update(acc, elem, :black, &(opposite_color(&1))) end)
  end
